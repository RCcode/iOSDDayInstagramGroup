...
static NSString* kRestserverOembedURL = @"http://api.instagram.com";
...
//    [params setValue:kSDKVersion forKey:@"sdk_version"];
    //lxf 0228
    NSArray *keysAll = [params allKeys];
    if ([keysAll containsObject:@"Relationship"]) {
        [params setValue:[params objectForKey:@"Relationship"] forKey:@"action"];
        [params removeObjectForKey:@"Relationship"];
        
    }
    //
    
    if (([self isSessionValid]) && (![url isEqualToString:@"http://api.instagram.com/oembed"])) {
 ...
 */
// lxf Step One: Direct your user to our authorization URL
- (void)authorizeWithSafary {
...
-(IGRequest*)requestWithMethodName:(NSString*)methodName
    ...
    NSString *fullURL = [NSString alloc];
    if ([methodName isEqual: @"/oembed"]) {
        fullURL = [kRestserverOembedURL stringByAppendingString:methodName];
    }
    else {
        fullURL = [kRestserverBaseURL stringByAppendingString:methodName];
    }
    
    NSLog(@"%@", fullURL);
    
   // NSString *fullURL = [[NSString alloc] initWithS]
